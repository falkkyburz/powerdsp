...
{
    f32_vbus_avg =
        pdsp_expavg(&f32_vbus_avg_state, &f32_vbus_avg_par, f32_vbus_act);
}
...
void test_saw(void)
{
    ...
    pdsp_saw_param_t s_saw_param = {.f32_step = PDSP_2_PI / 10,
                                    .f32_out_low = 0.0f,
                                    .f32_out_high = PDSP_2_PI};
    pdsp_saw_t s_saw = {0};
    ...
    for (u32_idx = 0; u32_idx < 100; u32_idx++)
        fprintf(fptr, "%i, %f, %f, %f\n", u32_idx, f32_arr[u32_idx],
                f32_arr1[u32_idx], sinf(f32_arr[u32_idx]));
    fclose(fptr);
}
...
void test_rolling_avg_rms(void)
{
    ...
    pdsp_saw_param_t s_saw_param = {.f32_step = PDSP_2_PI / 50,
                                    .f32_out_low = 0.0f,
                                    .f32_out_high = PDSP_2_PI};
    pdsp_saw_t s_saw = {0};
...
void test_avg(void)
{
    ...
    pdsp_saw_param_t s_saw_param = {.f32_step = PDSP_2_PI / 25,
                                    .f32_out_low = 0.0f,
                                    .f32_out_high = PDSP_2_PI};
    pdsp_saw_t s_saw = {0};
...
pdsp_u32_t test_brach(pdsp_u32_t u32_in) { return u32_in > 1000 ? 1 : 0; }
...
void test_i16_to_ascii(void)
{
    char mystr[128];
    char *pmystr = mystr;
    pmystr = pdsp_convert_i16_to_a6c(1234, pmystr);
    *(pmystr++) = ',';
    pmystr = pdsp_convert_i16_to_a6c(-1234, pmystr);
    *(pmystr++) = ',';
    pmystr = pdsp_convert_i16_to_a6c(32767, pmystr);
    *(pmystr++) = ',';
    pmystr = pdsp_convert_i16_to_a6c(-32768, pmystr);
    *(pmystr++) = '\n';
    pmystr = pdsp_convert_i16_to_a6c(1234, pmystr);
    *(pmystr++) = ',';
    pmystr = pdsp_convert_i16_to_a6c(-1234, pmystr);
