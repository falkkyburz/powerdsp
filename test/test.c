...
void test_raw(void)
{
   ...
   pdsp_raw_param_t s_raw_par = {.f32_gain = 2.0f, .f32_offset = -22.0f};
   ...
   for (u32_idx = 0; u32_idx < 100; u32_idx++)
   {
      if (u32_idx > 80)
         s_ovr.u32_enable = 1;
      f32_arr1[u32_idx] = pdsp_raw_run(&s_raw_par, &s_ovr, f32_arr[u32_idx]);
   }
...
void test_rolling_avg_rms(void)
{
   ...
   pdsp_saw_param_t s_saw_param = {.f32_step = PDSP_2_PI / 50, .f32_out_low = 0.0f, .f32_out_high = PDSP_2_PI};
   pdsp_saw_t s_saw = {0};
   pdsp_rolling_sum_t s_avg = {0};
   pdsp_rolling_sum_t s_rms = {0};

   FILE *fptr;
   fptr = fopen("test.csv", "w");
   for (u32_idx = 0; u32_idx < 200; u32_idx++)
   {
      f32_saw = pdsp_saw_run(&s_saw, &s_saw_param);
      f32_sin = sinf(f32_saw);
      f32_avg = pdsp_rolling_avg_run(&s_avg, af32_arr, 100, f32_saw);
      f32_rms = pdsp_rolling_rms_run(&s_rms, af32_arr1, 100, f32_sin);
...
void test_avg(void)
{
   ...
   pdsp_expavg_t s_avg = 0.0f;
   pdsp_expavg_param_t s_avg_param = 0.005f;
   ...
   {
      f32_saw = pdsp_saw_run(&s_saw, &s_saw_param);
      f32_avg = pdsp_expavg_run(&s_avg, &s_avg_param, f32_saw);
      fprintf(fptr, "%i, %f, %f\n",
              u32_idx,
              f32_saw,
              f32_avg);
